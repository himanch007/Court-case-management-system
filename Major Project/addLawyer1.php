...
<body>
<h1 style="text-align:center;text-transform:uppercase;">Lawyer Registration</h1>
<form method="post">
<div class="a">
...
if(isset($_POST['t1']))
{
	...
	$stmt = $conn->prepare("INSERT INTO Lawyer(ename, password) VALUES (?, ?)");
	$stmt->bind_param("ss", $first, $last);
	$first = $_POST['t1'];
	$last = $_POST['t2'];
	$stmt->execute();
	$q = "create table $first(eid INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
				ename VARCHAR(30),
				description VARCHAR(30),
				typeofcase VARCHAR(30),
				status VARCHAR(10),
				dateofhearing VARCHAR(10),
				caseaccepted VARCHAR(10)
				)";
